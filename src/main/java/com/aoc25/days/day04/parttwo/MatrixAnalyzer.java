package com.aoc25.days.day04.parttwo;
        ...
        while (true) {
            ...
            for (int indexX = 0; indexX < _matrix[0].length; indexX++) {
                for (int indexY = 0; indexY < _matrix.length; indexY++) {
                    ...
                    if (currentChar == '@') {
                        int checkedField = broadSearch(indexX, indexY);
                        if (checkedField == 1) {
                            positionsToChange.add(new Integer[] { indexX, indexY });
                            result++;
                        }
                    }
                }
            }
            if (positionsToChange.isEmpty()) {
                break;
            } else {
                for (Integer[] position : positionsToChange) {
                    _matrix[position[1]][position[0]] = '.';
                }
            }
        }
        return result;
